...
footfake = b"\x03\x00\xd7\xcd\xc6\x9a"
fakefoot = b"\x00\x00\x00\x00\x3A\x34\x03\x00"
...
while i != index:
	...
	if flip == 1:
		...
		if fakefoot == f.read(8) :
			...
			f.seek(hit)
			# above works
			waffles = open(str(hit)+".bobFak2.wmf", "wb")
			f.seek(hit)
			# f.seek(hit) is needed AGAIN else the blow fails and acts like it was f.seek(i)
			waffles.write(f.read(exlen))
			waffles.close()
